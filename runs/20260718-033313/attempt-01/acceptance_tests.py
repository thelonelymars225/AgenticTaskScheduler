...
def test_cli_valid():
    # Test CLI: merge two valid JSON files and write the result to a file
...
def test_cli_invalid():
    ...
    with tempfile.NamedTemporaryFile(mode='w', encoding='utf-8') as f1, \
        ...
        f2.seek(0)
        subprocess.run([sys.executable, os.environ['CANDIDATE_PATH'], f1.name, f2.name], stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
