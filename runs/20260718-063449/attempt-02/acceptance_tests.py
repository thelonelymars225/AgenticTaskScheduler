...
def test_merge_json_invalid():
    # Test invalid case: merge two JSON objects with a conflict
    left_data = {'a': 1}
    right_data = {'a': 2}  # Conflict on key 'a'
    output_path = Path(tempfile.mkstemp()[1])
    try:
        candidate_module.merge(left_data, right_data)
    except Exception as e:
        assert str(e) == "Conflict on key 'a'"
    else:
        assert False, "Expected exception not raised"

def test_merge_json_cli():
    # Test CLI: merge two JSON files and write output to a file
...
def test_merge_json_cli_invalid():
    # Test CLI: merge two JSON files and write output to a file (invalid case)
    left_path = Path(tempfile.mkstemp()[1])
    right_path = Path(tempfile.mkstemp()[1])
    with open(left_path, 'w', encoding='utf-8') as f:
        json.dump({'a': 1}, f)
