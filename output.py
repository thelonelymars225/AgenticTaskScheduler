...
class PongGame:
    def __init__(self, root):
        ...
        self.root.bind("<KeyPress-Left>", lambda _: self.paddle_left.move(-10))
        self.root.bind("<KeyPress-Right>", lambda _: self.paddle_left.move(10))
        self.root.bind("<KeyPress-a>", lambda _: self.paddle_right.move(-10))
        self.root.bind("<KeyPress-d>", lambda _: self.paddle_right.move(10))
...
class Paddle:
    def __init__(self, canvas, side):
        self.canvas = canvas
        if side == "left":
            self.id = canvas.create_rectangle(0, 150, 20, 250, fill="white", tags="paddle_left")
        else:
            self.id = canvas.create_rectangle(580, 150, 600, 250, fill="white", tags="paddle_right")
    ...
    def move(self, distance):
        pos = self.canvas.coords(self.id)
        if pos[1] + distance >= 0 and pos[3] + distance <= 400:
